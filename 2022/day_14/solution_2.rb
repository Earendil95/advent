...
shift = all_points.min_by(&:first).first - 2 
...
(0..).each do |i|
  break if map[starting_point.first][starting_point.last] != EMPTY_CELL
  ...
  loop do
    ...
    if map[sand_unit.first][sand_unit.last + 1] == EMPTY_CELL
      ...
      map.unshift(COLUMN.dup)
      sand_unit[0] += 1
      starting_point[0] += 1
    elsif sand_unit.first < map.size - 1 && map[sand_unit.first + 1][sand_unit.last + 1] == EMPTY_CELL
      sand_unit = [sand_unit.first + 1, sand_unit.last + 1]
  ...
  # print_map(map) && sleep(0.4) if i % 200 == 0
end
